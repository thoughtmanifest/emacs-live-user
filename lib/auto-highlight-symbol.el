...
(defcustom ahs-default-range 'ahs-range-whole-buffer
  "Default Plugin."
  :group 'auto-highlight-symbol
...
(if auto-highlight-symbol-mode-map
  ...
  (setq auto-highlight-symbol-mode-map
        (let ((map (make-sparse-keymap)))
          (define-key map (kbd "M-s-<left>"    ) 'ahs-backward            )
          (define-key map (kbd "M-s-<right>"   ) 'ahs-forward             )
          (define-key map (kbd "M-S-s-<left>"  ) 'ahs-backward-definition )
          (define-key map (kbd "M-S-s-<right>" ) 'ahs-forward-definition  )
